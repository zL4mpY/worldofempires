...
silver_texture = pygame.image.load(get_base_dir() / "worldofempires" / "textures" / "silver_ore.png", "")
width = silver_texture.get_width()
height = silver_texture.get_height()

class SilverOre(BaseObject):
    def __init__(self, game, scene, x, y):
        ...
        self.surface = pygame.transform.scale(silver_texture, (new_width, new_height))
    ...
    def destroy(self, destroyer):
        destroyer.inventory.add_item(4, self.amount)
        destroyer.homeland.inventory.add_item(4, self.amount)
        destroyer.country.inventory.add_item(4, self.amount)
