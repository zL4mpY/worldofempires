...
tree_texture = pygame.image.load(get_base_dir() / "worldofempires" / "textures" / "tree.png", "")
width = tree_texture.get_width()
height = tree_texture.get_height()
...
class Tree(BaseObject):
    def __init__(self, game, scene, x, y):
        ...
        self.surface = pygame.transform.scale(tree_texture, (new_width, new_height))
    ...
    def destroy(self, destroyer):
        destroyer.inventory.add_item(1, self.amount)
        destroyer.homeland.inventory.add_item(1, self.amount)
        destroyer.country.inventory.add_item(1, self.amount)
