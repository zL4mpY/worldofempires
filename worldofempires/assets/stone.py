...
stone_texture = pygame.image.load(get_base_dir() / "worldofempires" / "textures" / "stone.png", "")
width = stone_texture.get_width()
height = stone_texture.get_height()
...
class Stone(BaseObject):
    def __init__(self, game, scene, x, y):
        ...
        self.surface = pygame.transform.scale(stone_texture, (new_width, new_height))
    ...
    def destroy(self, destroyer):
        destroyer.inventory.add_item(2, self.amount)
        destroyer.homeland.inventory.add_item(2, self.amount)
        destroyer.country.inventory.add_item(2, self.amount)
