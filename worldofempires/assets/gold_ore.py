...
gold_ore_texture = pygame.image.load(get_base_dir() / "worldofempires" / "textures" / "gold_ore.png", "")
width = gold_ore_texture.get_width()
height = gold_ore_texture.get_height()

class GoldOre(BaseObject):
    def __init__(self, game, scene, x, y):
        ...
        self.surface = pygame.transform.scale(gold_ore_texture, (new_width, new_height))
    ...
    def destroy(self, destroyer):
        destroyer.inventory.add_item(5, self.amount)
        destroyer.homeland.inventory.add_item(5, self.amount)
        destroyer.country.inventory.add_item(5, self.amount)
