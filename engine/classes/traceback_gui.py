...
class TracebackPlusWindow(Toplevel):
    def __init__(self, master=None):
        ...
        def copy_log(window):
            # current_tab_index = window.notebook.index(window.notebook.select())
            # current_tab_name = window.notebook.tab(current_tab_index)[1]['text']
            # page = window.notebook.nametowidget(current_tab_name)
            # text = page['state'] == 'normal' and page.get('1.0', END) or ''
            pyperclip.copy(all_tracebacks)
        ...
        # Add Copy Log button
        copy_button = Button(self, text="Copy Log", command=lambda: copy_log(self))
        copy_button.pack(fill='x', padx=4, pady=4)

        # Add Exit button
        exit_button = Button(self, text="Exit", command=exit_application)
        exit_button.pack(fill='x', padx=4, pady=4)
        self.protocol('WM_DELETE_WINDOW', exit_application)
    ...
    def _show_traceback(self, exception: BaseException, additional_message=None):
        ...
        for tb in _iter_tb(exception.__traceback__):
            ...
            all_tracebacks += '  File "'
            all_tracebacks += str(filename)
            all_tracebacks += '", line '
            all_tracebacks += str(lineno)
            all_tracebacks += ', in '
            all_tracebacks += str(scope)
            all_tracebacks += '\n'
            
            for lineno, code_line in _get_code(filename, lineno):
                all_tracebacks += '{:7d} '.format(lineno)
                all_tracebacks += code_line
                all_tracebacks += '\n'
            all_tracebacks += '\n'
        ...
        all_tracebacks += str(_get_type(exception))
        all_tracebacks += ': '
        all_tracebacks += str(exception)
        all_tracebacks += '\n'
